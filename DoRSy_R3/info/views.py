...
@login_required()
def post_disable_course_stream(request, c_class_id, stud_id):
	c_class = Class.objects.get(id=c_class_id)
	stud = Student.objects.get(id=stud_id)
	att_list1 = Question.objects.filter(q_class=c_class).order_by('-date')
	att_list2 = Class.objects.filter(course=c_class.course).order_by('-year')
	att_list3 = stud
	att_list4 = c_class
	return render(request, 'info/post_disable_course_stream.html', {'att_list1' : att_list1, 'att_list2' : att_list2, 'att_list3' : att_list3, 'att_list4' : att_list4})
...
@login_required()
def t_post_disable_course_stream(request, c_class_id, teach_id):
	c_class = Class.objects.get(id=c_class_id)
	teach = Teacher.objects.get(id=teach_id)
	att_list1 = Question.objects.filter(q_class=c_class).order_by('-date')
	att_list2 = Class.objects.filter(course=c_class.course).order_by('-year')
	att_list3 = teach
	att_list4 = c_class
	return render(request, 'info/t_post_disable_course_stream.html', {'att_list1' : att_list1, 'att_list2' : att_list2, 'att_list3' : att_list3, 'att_list4' : att_list4})
...
@login_required()
def post_disable_viewques(request, q_id, u_id, p_id):
	q = Question.objects.get(id=q_id)
	ans = Answer.objects.filter(ques=q)
	return render(request, 'info/post_disable_question.html', {'att_list1' : q, 'att_list2' : u_id, 'att_list3' : ans, 'id' : p_id})
...
@login_required()
def t_post_disable_viewques(request, q_id, u_id, p_id):
	q = Question.objects.get(id=q_id)
	ans = Answer.objects.filter(ques=q)
	return render(request, 'info/t_post_disable_question.html', {'att_list1' : q, 'att_list2' : u_id, 'att_list3' : ans, 'id' : p_id})
